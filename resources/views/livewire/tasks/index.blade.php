<div>
    ...
    <div class="grid grid-cols-1 items-start gap-3 md:grid-cols-2 xl:grid-cols-3">
        @foreach ($tasks as $task)
            @php
                $isDone = isset($completedIds[$task['id']]);
                $hasFir = collect($task['objectives'] ?? [])->contains(fn ($o) => ! empty($o['foundInRaid']));
            @endphp
            <details wire:key="task-{{ $task['id'] }}" class="group rounded-xl border bg-[#14171f] {{ $isDone ? 'border-emerald-900/60 opacity-60' : 'border-zinc-800' }} open:border-zinc-700">
                <summary class="flex cursor-pointer items-center gap-3 p-4">
                    ...
                    <span class="flex shrink-0 gap-1.5">
                        ...
                        @endif
                        @if ($hasFir)
                            <span class="rounded-full bg-emerald-400/10 px-2 py-0.5 text-xs font-semibold text-emerald-300" title="Exige item encontrado em raid (Found in Raid)">FiR</span>
                        @endif
                    </span>
                    ...
                    @if (! empty($task['objectives']))
                        ...
                        <ul class="mb-3 list-inside list-disc space-y-1 text-zinc-300">
                            @foreach ($task['objectives'] as $obj)
                                <li>
                                    {{ $obj['description'] }}
                                    @if (! empty($obj['foundInRaid']))
                                        <span class="ml-1 rounded bg-emerald-400/10 px-1.5 py-0.5 text-[10px] font-semibold text-emerald-300" title="Item precisa ser encontrado em raid">FiR</span>
                                    @endif
