<div>
    ...
    <div class="columns-1 gap-3 md:columns-2 xl:columns-3">
        @foreach ($stations as $station)
            @php $isOpen = $searching || $open === $station['id']; @endphp
            <div wire:key="station-{{ $station['id'] }}" class="mb-3 break-inside-avoid rounded-xl border bg-[#14171f] {{ $isOpen ? 'border-amber-500/30' : 'border-zinc-800' }}">
                <button type="button" wire:click="toggle('{{ $station['id'] }}')"
                    ...
                    @endif
                    <span class="flex-1 font-semibold {{ $isOpen ? 'text-amber-300' : 'text-zinc-100' }}">{{ $station['name'] }}</span>
                    <span class="text-xs text-zinc-500">{{ count($station['levels'] ?? []) }} {{ $searching ? 'nível(is) com o item' : 'níveis' }}</span>
                    <span class="text-zinc-600 transition {{ $isOpen ? 'rotate-90' : '' }}">▸</span>
                </button>
                                        ...
                                        @foreach ($level['itemRequirements'] as $req)
                                            ...
                                            <li class="flex items-center gap-2 rounded text-sm {{ $isMatch ? 'bg-amber-400/10 px-1 py-0.5 font-medium text-amber-300' : 'text-zinc-300' }}">
                                                @if (! empty($req['item']['iconLink']))
                                                    <img src="{{ $req['item']['iconLink'] }}" alt="" class="h-6 w-6 rounded bg-zinc-900 object-contain" loading="lazy">
                                                @endif
                                                <span class="flex-1">
